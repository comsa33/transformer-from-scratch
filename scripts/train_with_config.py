...
from configs.utils import load_config, save_config  # noqa: E402
from data.dataset import TranslationDataset  # noqa: E402
from data.tokenizer import SimpleTokenizer  # noqa: E402
from evaluation.metrics import MetricsTracker  # noqa: E402
from training.optimizer import get_optimizer_and_scheduler  # noqa: E402
from training.trainer import TransformerTrainer  # noqa: E402
from transformer.models.transformer import Transformer  # noqa: E402
...
def create_dataloaders(config, tokenizer):
    ...
    train_dataset = TranslationDataset(
        src_texts[:80], tgt_texts[:80], tokenizer, tokenizer, max_length=config.data.max_length
    )
...
def main():
    ...
    parser.add_argument(
        "--config", "-c", type=str, default="base", help="설정 파일 이름 또는 경로 (기본값: base)"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    # 토크나이저 생성
    vocab = ["<pad>", "<unk>", "<sos>", "<eos>"] + [f"token_{i}" for i in range(1000)]
