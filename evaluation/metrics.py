...
try:
    ...
    warnings.warn("scikit-learn not installed. Some metrics may not be available.")
    accuracy_score = None
    f1_score = None
...
try:
    ...
    warnings.warn("rouge-score not installed. ROUGE metrics will not be available.")
    rouge_scorer = None

try:
    import sacrebleu
except ImportError:
    warnings.warn("sacrebleu not installed. BLEU metrics will not be available.")
    sacrebleu = None
...
def get_metrics_for_task(task: str) -> MetricCollection:
    ...
    if task == "classification":
        ...
        warnings.warn(f"Unknown task: {task}. Using default metrics.")
        if accuracy_score is not None:
