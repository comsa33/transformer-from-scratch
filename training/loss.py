...
class CrossEntropyLoss(nn.Module):
    ...
    def __init__(
        self, ignore_index: int = -100, reduction: str = "mean", label_smoothing: float = 0.0
    ):
        super().__init__()
    ...
    def forward(
        self, logits: torch.Tensor, targets: torch.Tensor, mask: torch.Tensor | None = None
    ) -> torch.Tensor:
        """
        Args:
...
class MaskedLanguageModelingLoss(nn.Module):
    ...
    def __init__(self, vocab_size: int, ignore_index: int = -100, label_smoothing: float = 0.0):
        super().__init__()
        self.vocab_size = vocab_size
        self.criterion = CrossEntropyLoss(
            ignore_index=ignore_index, reduction="none", label_smoothing=label_smoothing
        )

    def forward(
        self, predictions: torch.Tensor, labels: torch.Tensor, masked_positions: torch.Tensor
