...
class WarmupScheduler(_LRScheduler):
    ...
    def __init__(self, optimizer: optim.Optimizer, warmup_steps: int, last_epoch: int = -1):
...
class TransformerScheduler(WarmupScheduler):
    ...
    def get_lr(self) -> list[float]:
        ...
        if step < self.warmup_steps:
            # Warmup phase
            return [
                base_lr * scale * step * (self.warmup_steps ** (-1.5)) for base_lr in self.base_lrs
            ]
        else:
            # Decay phase
            return [base_lr * scale * (step ** (-0.5)) for base_lr in self.base_lrs]
...
class PolynomialDecayScheduler(WarmupScheduler):
    ...
    def get_lr(self) -> list[float]:
        if self.last_epoch < self.warmup_steps:
            ...
            return [
                self.end_lr + (base_lr - self.end_lr) * decay_factor for base_lr in self.base_lrs
            ]
...
class OneCycleLR(_LRScheduler):
    ...
    def get_lr(self) -> list[float]:
        ...
        # Phase 2: 감소 구간
            ...
            return [
                max_lr * pct + (max_lr / self.final_div_factor) * (1 - pct)
...
class CyclicLRWithWarmup(_LRScheduler):
    ...
    def get_lr(self) -> list[float]:
        ...
        return [
            base_lr + (max_lr - base_lr) * max(0, (1 - x)) * scale
    ...
    **kwargs,
    ...
    if optimizer_type.lower() == "adam":
        optimizer = optim.Adam(
            optimizer_grouped_parameters, lr=learning_rate, betas=betas, eps=eps, **kwargs
        )
        ...
        optimizer = optim.Adagrad(optimizer_grouped_parameters, lr=learning_rate, eps=eps, **kwargs)
    else:
    ...
    **kwargs,
    ...
    if scheduler_type == "transformer":
        return TransformerScheduler(
            optimizer, d_model=kwargs.get("d_model", 512), warmup_steps=warmup_steps
        )
    elif scheduler_type == "linear":
        return LinearWarmupScheduler(optimizer, warmup_steps=warmup_steps)
        ...
        return ExponentialWarmupScheduler(
            optimizer, warmup_steps=warmup_steps, gamma=kwargs.get("gamma", 0.95)
        )
