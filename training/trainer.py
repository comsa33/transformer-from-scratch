...
class Trainer:
    ...
    def __init__(
        ...
        eval_dataset: torch.utils.data.Dataset | None = None,
        data_collator: Callable | None = None,
        tokenizer: Any | None = None,
        compute_metrics: Callable | None = None,
        callbacks: list[Callable] | None = None,
        optimizers: tuple[
            torch.optim.Optimizer | None, torch.optim.lr_scheduler._LRScheduler | None
        ] = (None, None),
    ):
        self.model = model
        self.args = args
    ...
    def train(self):
        ...
        for epoch in range(self.args.num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                epoch_iterator.set_description(f"Epoch {epoch} - Loss: {tr_loss_step.item():.4f}")
        ...
        if self.args.load_best_model_at_end and self.state.best_model_checkpoint:
            if os.path.exists(self.state.best_model_checkpoint):
                ...
                self.logger.warning(
                    f"Best model checkpoint not found: {self.state.best_model_checkpoint}"
                )

        return self.state.log_history

    def evaluate(self, eval_dataset: torch.utils.data.Dataset | None = None) -> dict:
        """모델 평가"""
        eval_dataloader = (
            self.get_eval_dataloader()
            if eval_dataset is None
            else DataLoader(eval_dataset, batch_size=self.args.per_device_eval_batch_size)
        )
        ...
        self.state.log_history.append(
            {**eval_metrics, "epoch": self.state.epoch, "step": self.state.global_step}
        )
    ...
    def _save_checkpoint(self):
        ...
        # 학습 상태 저장
        self.state.save_to_json(os.path.join(checkpoint_path, "trainer_state.json"))
    ...
    def _load_checkpoint(self, checkpoint_path: str):
        """체크포인트 로드"""
        # 모델 로드
        self.model.load_state_dict(torch.load(os.path.join(checkpoint_path, "pytorch_model.bin")))
        ...
        if os.path.exists(os.path.join(checkpoint_path, "trainer_state.json")):
            self.state = TrainerState.from_json(os.path.join(checkpoint_path, "trainer_state.json"))

        self.logger.info(f"Checkpoint loaded from {checkpoint_path}")

    def _get_checkpoint_path(self) -> str:
        """체크포인트 경로 생성"""
        return os.path.join(self.args.output_dir, f"checkpoint-{self.state.global_step}")
