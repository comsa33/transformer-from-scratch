...
class DataCollatorForLanguageModeling:
    ...
    def _get_special_tokens_mask(self, inputs: torch.Tensor) -> torch.Tensor:
        """특수 토큰 마스크 생성"""
        special_tokens_ids = [self.tokenizer.pad_token_id, self.tokenizer.bos_token_id, 
...
class DataCollatorForSeq2Seq:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        if sources:
            ...
            if self.pad_to_multiple_of:
                max_source_length = ((max_source_length + self.pad_to_multiple_of - 1) // 
                                   self.pad_to_multiple_of) * self.pad_to_multiple_of
...
class LanguageModelingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return {
            ...
            "attention_mask": [1] * len(self.examples[idx])
        }
