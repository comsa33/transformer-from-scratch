...
class ConfigDict(dict):
    ...
    def get_model_config(self) -> TransformerConfig:
        ...
        model_dict = self['model'].copy() if isinstance(self['model'], dict) else asdict(self['model'])
...
def load_config(config_name_or_path: Union[str, Path]) -> ConfigDict:
    """설정 파일 로드
    ...
    """
    # 설정 이름으로 전달된 경우
    if isinstance(config_name_or_path, str) and not config_name_or_path.endswith(('.yaml', '.yml', '.json')):
...
def save_config(config: Union[ConfigDict, dict], save_path: Union[str, Path]) -> None:
    """설정을 파일로 저장
...
def merge_configs(*configs: Union[ConfigDict, dict, str, Path]) -> ConfigDict:
    """여러 설정을 병합
