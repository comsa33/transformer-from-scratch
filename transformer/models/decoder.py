...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if return_attention:
            ...
            normalized = (
                self.self_attn_residual.norm(x) if hasattr(self.self_attn_residual, "norm") else x
            )
            ...
            self_attn_out = self.self_attn_residual(
                x, lambda x: self.self_attention(x, x, x, self_attn_mask)
            )
        ...
        if return_attention:
            ...
            normalized = (
                self.cross_attn_residual.norm(self_attn_out)
                if hasattr(self.cross_attn_residual, "norm")
                else self_attn_out
            )
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.embeddings = PositionalTokenEmbedding(
            vocab_size=vocab_size, d_model=d_model, max_seq_length=max_length, dropout=dropout
        )
        ...
        if share_embeddings:
            ...
            self.output_projection = lambda x: torch.matmul(
                x, self.embeddings.token_embedding.weight.T
            )
        else:
