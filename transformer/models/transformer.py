...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        share_encoder_decoder_embeddings: bool | None = None,
    ):
        super().__init__()
    ...
    def _beam_search(
        ...
        # 초기 시퀀스
        sequences = torch.full((batch_size * beam_size, 1), start_token_id, device=device)
        ...
        for step in range(max_length - 1):
            ...
            sequences = torch.cat(
                [sequences[beam_indices.flatten()], next_tokens.flatten().unsqueeze(1)], dim=1
            )
