...
class EncoderLayer(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, mask: torch.Tensor | None = None, return_attention: bool = False
    ) -> torch.Tensor:
        """
        ...
        if return_attention:
            ...
            attn_out, attn_weights = self.self_attention(
                normalized, normalized, normalized, mask, return_attention=True
            )
            attn_out = x + self.attn_residual.dropout(attn_out)
        else:
            attn_out = self.attn_residual(x, lambda x: self.self_attention(x, x, x, mask))
            attn_weights = None
    ...
    def get_attention_maps(
        ...
        with torch.no_grad():
            if layer_idx is not None:
                ...
                _, attentions = self.forward(input_ids, mask, return_attention=True)
                return attentions
