...
def he_uniform_(
    tensor: torch.Tensor, a: float = 0, mode: str = "fan_in", nonlinearity: str = "relu"
) -> torch.Tensor:
...
def init_transformer_params(module: nn.Module, d_model: int, init_type: str = "xavier_uniform"):
...
def _calculate_gain(nonlinearity: str, param: float | None = None) -> float:
    """활성화 함수에 따른 gain 계산"""
    linear_fns = [
        "linear",
        "conv1d",
...
class TransformerInitializer:
    ...
    def _init_norm(self, module: nn.LayerNorm | nn.GroupNorm):
