...
from evaluation.metrics import (
    AccuracyMetric, F1Metric, PerplexityMetric,
    BLEUMetric, ROUGEMetric, ExactMatchMetric,
    TokenAccuracyMetric, MetricCollection,
    get_metrics_for_task, EarlyStopping
)
...
def test_token_accuracy_metric():
    ...
    references = torch.tensor([
        [1, 2, 3, 4, -100],  # 마지막 토큰은 무시
        [6, 7, 9, 9, 10]     # 3번째 토큰 틀림
    ])
...
def visualize_confusion_matrix():
    ...
    for i in range(n_classes):
        for j in range(n_classes):
            ax2.text(j, i, f'{confusion_matrix_normalized[i, j]:.2f}',
                    ha="center", va="center", color="black")
