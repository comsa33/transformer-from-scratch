...
def test_padding_mask():
    ...
    print("마스크 (batch_size x 1 x 1 x seq_len):")
    ...
    print("샘플 0의 첫 번째 행 attention weights:")
    print(f"  {attention_weights[0, 0, 0].tolist()}")
    print("  (패딩 위치의 가중치가 0에 가까움)")
...
def test_combined_mask():
    ...
    # 패딩이 있는 시퀀스
    seq = torch.tensor([[1, 2, 3, 4, 0, 0], [1, 2, 3, 4, 5, 6]])  # 패딩 없음
...
def test_cross_attention_mask():
    ...
    target_seq = torch.tensor(
        [[1, 2, 3, 0, 0, 0], [1, 2, 3, 4, 5, 6]]  # Decoder: 3개 토큰  # Decoder: 6개 토큰
    )
