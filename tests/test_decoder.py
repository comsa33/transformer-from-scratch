...
from transformer.models.decoder import DecoderLayer, create_decoder
from transformer.models.encoder import create_encoder
from transformer.utils.masking import create_look_ahead_mask
...
def test_decoder_layer():
    ...
    # Decoder layer 생성
    decoder_layer = DecoderLayer(d_model=d_model, num_heads=num_heads, d_ff=d_ff, dropout=0.0)
...
def test_full_decoder():
    ...
    print(f"Decoder 출력 shape: {output.shape}")
    print(
        f"예상 출력 shape: [batch_size={batch_size}, tgt_seq={tgt_seq_length}, vocab_size={vocab_size}]"
    )
...
def visualize_attention_patterns(decoder, tgt_ids, encoder_output, src_mask, tgt_mask):
    ...
    for i in range(self_attn.shape[0]):
        for j in range(i + 1, self_attn.shape[1]):
            ax1.add_patch(
                plt.Rectangle((j - 0.5, i - 0.5), 1, 1, fill=True, color="gray", alpha=0.3)
            )
...
def test_generation():
    ...
    encoder = create_encoder(
        num_layers=2, d_model=d_model, num_heads=num_heads, vocab_size=vocab_size
    )
...
def visualize_generation_process(decoder, encoder_output, generated):
    ...
    with torch.no_grad():
        _, self_attns, cross_attns = decoder(generated, encoder_output, return_attention=True)
...
def analyze_layer_contributions():
    ...
    for i, layer in enumerate(decoder.layers):
        ...
        print(
            f"Layer {i + 1} - Self: {self_change:.4f}, Cross: {cross_change:.4f}, FFN: {ffn_change:.4f}"
        )
...
if __name__ == "__main__":
    ...
    # 2. Full Decoder 테스트
    encoder, decoder, src_ids, tgt_ids, src_mask, tgt_mask, encoder_output, output = (
        test_full_decoder()
    )
