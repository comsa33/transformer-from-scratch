...
from transformer.utils.initialization import (
    ...
    he_uniform_, he_normal_,
    normal_, uniform_, truncated_normal_,
    init_bert_params, init_gpt2_params, init_transformer_params,
...
def visualize_initialization_distributions(tensors: dict):
    ...
    for idx, (name, tensor) in enumerate(tensors.items()):
        ...
        ax.axvline(mean + std, color='green', linestyle='--', linewidth=1, label=f'±1 STD: {std:.4f}')
...
def test_model_initialization():
    ...
    for method in init_methods:
        ...
        print(f"{method:<15} | {linear_weights.mean():.6f}    | "
...
def test_transformer_initializer():
    ...
    for strategy in strategies:
        ...
        initializer = TransformerInitializer(
            d_model=512,
            n_layers=6,
            init_type=strategy
        )
...
def compare_initialization_effects():
    ...
    for name, stats in results.items():
        print(f"{name:<15} | {stats['mean']:>10.4f} | {stats['std']:>10.4f} | {stats['max']:>10.4f}")
