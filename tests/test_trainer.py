...
from training.trainer import Trainer, TrainingConfig, TrainerState
...
class DummyTranslationDataset(Dataset):
    ...
    def __init__(self, size: int = 1000, src_len: int = 20, tgt_len: int = 25, 
                 vocab_size: int = 1000):
        self.size = size
...
def test_checkpoint_save_load():
    ...
    if checkpoints:
        ...
        # 새 모델과 trainer
        model2 = create_transformer_small(
            src_vocab_size=100,
            tgt_vocab_size=100,
            d_model=64
        )
