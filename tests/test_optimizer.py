...
def visualize_scheduler_curves():
    ...
    schedulers = {
        "Transformer": TransformerScheduler(
            torch.optim.Adam(model.parameters(), lr=base_lr), d_model=512, warmup_steps=warmup_steps
        ),
        ...
        "Exponential": ExponentialWarmupScheduler(
            torch.optim.Adam(model.parameters(), lr=base_lr), warmup_steps=warmup_steps, gamma=0.995
        ),
    }
...
def test_onecycle_lr():
    ...
    scheduler = OneCycleLR(
        optimizer, max_lr=max_lr, total_steps=total_steps, pct_start=0.3, anneal_strategy="cos"
    )
    ...
    plt.axvline(
        x=peak_step, color="red", linestyle="--", alpha=0.5, label=f"Peak (step {peak_step})"
    )
...
def compare_warmup_strategies():
    ...
    for warmup in warmup_lengths:
        ...
        scheduler = CosineAnnealingWarmupScheduler(
            optimizer, warmup_steps=warmup, max_steps=5000, eta_min=1e-5
        )
...
def test_scheduler_with_training():
    ...
    scheduler = create_scheduler(optimizer, "cosine", warmup_steps=100, max_steps=500)
...
def analyze_transformer_schedule():
    ...
    for d_model in d_models:
        ...
        scheduler = TransformerScheduler(optimizer, d_model=d_model, warmup_steps=warmup_steps)
    ...
    print(
        "Transformer schedule 분석이 'outputs/transformer_schedule_analysis.png'에 저장되었습니다."
    )
